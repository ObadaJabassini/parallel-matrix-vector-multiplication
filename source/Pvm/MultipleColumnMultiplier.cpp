...
MultipleColumnMultiplier::MultipleColumnMultiplier( std::string file_path, int cols ) : MatrixMultiplier(file_path) {
    this->cols = cols;
}

void MultipleColumnMultiplier::multiply( std::string result_file_path ) {
    system(("../../bin/multiple_columns " + file_path + " " + std::to_string(cols) + " " + result_file_path).c_str());
}

std::string MultipleColumnMultiplier::multiply( bool justTime ) {
    string temp = "/tmp/temp.txt";
    system(("../../bin/multiple_columns " + file_path + " " + temp).c_str());
    string result = "";
    ifstream file;
    file.open(temp);
    std::string line;
